...
```diff
- Мой email john.doe@example.com, карта 4111 1111 1111 1111      # что отправил клиент
+ Мой email <EMAIL_1>, карта <CREDIT_CARD_1>                     # что увидела модель
```

…а в ответе модели прокси восстанавливает оригиналы — включая SSE-стриминг
токен-за-токеном и аргументы tool-call. Клиенты обращаются к `guardrails-llm-filter`
вместо провайдера; на пути к модели тело запроса сканируют ~260 встроенных
regex-правил (учётные данные, API-ключи, access-токены, IP-адреса, российские PII
с валидацией контрольных сумм) и заменяют найденное плейсхолдерами вида `<EMAIL_1>`.
